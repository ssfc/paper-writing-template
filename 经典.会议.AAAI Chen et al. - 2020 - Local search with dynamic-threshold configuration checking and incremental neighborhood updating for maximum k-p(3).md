...
previous methods => shortcomings of previous methods => this paper's method Component 2 (引出本文的方法)

construct this paper's method from Component 1 and Component 2 => experimental result (实验结果)

### Problem Notation

Graph => edge => vertex => neighbor => degree => edge density => set of neighbors => complement graph （基本组件符号）

k-plex => saturated set （问题相关符号）

local search => AddSet(S) \& SwpSet(S) \& PerturbSet(S) （方法相关符号）

### This paper's method two components
