...
problem investigated => this paper's method => previous methods' shortcomings v.s. this paper method's advantages => experimental result

### Introduction & Related Work

research field of problem => parent problem => this paper's problem => this paper's problem applications (引出所研究的问题)

challenge faced => previous methods to address challenges => methods categories => Category 1's methods => Category 1 methods' shortcomings => Category 2 methods => Category 2 methods' shortcomings => methods to address Category 2's shortcomings => shortcomings of methods to address Category 2's shortcomings => this paper's method Component 1 (引出本文的方法)

previous methods => shortcomings of previous methods => this paper's method Component 2

construct this paper's method from Component 1 and Component 2 => experimental result (方法效果)

### Problem Notation

Graph => edge => vertex => neighbor => degree => edge density => set of neighbors => complement graph （基本组件概念）

k-plex => saturated set （问题相关概念）

local search => AddSet(S) \& SwpSet(S) \& PerturbSet(S) （方法相关概念）

### Mathematical principles of this paper's method

Component 1: CC => variants of CC => variants shortcomings => this paper's method => Rules \& Lemmas （论文方法是某种方法的变种）

Component 2: incremental neighborhood updating scheme => this paper's incremental neighborhood updating scheme => implementation detail (Data Structures & Updating Rules & Complexity Analysis)  （论文方法是一般性方法的特例）
