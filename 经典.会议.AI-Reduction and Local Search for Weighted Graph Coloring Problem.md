...
本文研究了加权图着色问题（WGCP）的求解技术，这是图着色问题（GCP）的一个重要扩展，具有广泛的应用。本文提出了一个基于约简和局部搜索的算法，称为RedLS。RedLS算法包括两个阶段：

- 在第一阶段，本文提出了一个基于团采样的下界方法和约简规则，可以显著地减少图的大小。
- 在第二阶段，本文设计了一个基于两种选择规则和配置检查的局部搜索策略，可以有效地探索可行解和非可行解之间的转换。

本文在传统的基准图和大规模的社交网络图上进行了实验，结果表明RedLS算法显著优于现有的WGCP算法，在所有的基准图上都找到了更好的解。
